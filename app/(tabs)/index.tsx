...
// استيراد الـ Services الجديدة والمعدلة بالكامل
      ...
      const fetchData = async () => {
        try {
          ...
          if (userDoc.exists()) {
            ...
            // ✨ إضافة: تسجيل وإعداد الإشعارات تلقائياً للمهني عند دخول الواجهة
...
setBookings(data.sort((a, b) => {
  ...
  // دالة تحديث الحالة مع إرسال نوتيفيكاسيون للزبون (تعديل حقل expoPushToken)
    ...
    try {
      ...
      if (bookingItem.userId) {
        ...
        if (customerDoc.exists() && customerDoc.data().expoPushToken) {
          ...
          // تعديل الدالة لتتوافق مع البارامترات المستقرة للمهني والزبون
          await sendPushNotification(clientToken, proName);
        }
      }
    } catch (error) {
      console.error(error);
  ...
  // دالة إرسال التقييم المحدثة باستعمال الـ Transaction والـ Service الجديد
  const submitRating = async () => {
    ...
    try {
      ...
      // 1. تحديث الموعد كتقييم داخلي
      await updateDoc(doc(db, "bookings", bookingId), {
        rating: ratingScore,
        comment: comment,
      });

      // 2. استدعاء السيرفيس الاحترافي لتحديث كوليكشن reviews وحساب المعدل أوتوماتيكياً فـ users
  ...
  return (
    <View style={styles.container}>
      ...
      {/* Modal التقييم المعزز بنظام النجوم الفعلي */}
...
const styles = StyleSheet.create({
  ...
  ratingInput: { width: '100%', marginBottom: 15, textAlign: 'right' },
